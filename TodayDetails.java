...
import static anulom.executioner5.com3.anulom.GenericMethods.TABLE_CHECK_JOB;
import static anulom.executioner5.com3.anulom.Login.umailid;
import static anulom.executioner5.com3.anulom.R.drawable.calendar;
import static anulom.executioner5.com3.anulom.R.drawable.notify_1;
...
import static anulom.executioner5.com3.anulom.database.DBManager.TableInfo.ID1;
import static anulom.executioner5.com3.anulom.database.DBManager.TableInfo.TABLE_NAME;

public class TodayDetails extends Fragment implements IFetchCurrentLocation {
...
//        database =new DBOperation(this);
        ...
        // AutoRefresh();
        tasklist1 = (ListView) rootView.findViewById(R.id.tasklist);
        reFreshReload();

        if (GenericMethods.ctr2 == 0) {
//
        ...
        GenericMethods.ctr2 = GenericMethods.ctr2 + 1;
        return rootView;
                        ...
                        if (getAlldataList.get(position).get("ocontact").equals(getAlldataList.get(position).get("contact_no"))) {
                            ...
                            contactperson.setOnClickListener(new OnClickListener() {
                                ...
                                public void onClick(View v) {
                                    ...
                                    Cursor cursorw = sqldb1.query(CALL, null, conditionw, new String[]{DBManager.TableInfo.actual_time}, null, null, null);
                                    sqldb1.insert(CALL, null, values1);
                                    //System.out.println("time:"+timevalue1+getAlldataList.get(position).get("appid")+String.valueOf(currentTime).substring(10,20));
                                    cursorw.close();
                                    sqldb1.close();
                                    Intent intent1 = new Intent(getActivity().getApplicationContext(), call.class);
                                    getActivity().getApplicationContext().startService(intent1);
...
//
                ...
                //System.out.println("apptypefromtoday:"+getAlldataList.get(position).get("apptype"));
                if (getAlldataList.get(position).get("apptype").equals("1")) {
                    ...
                    //System.out.println("biocontent_today:"+getAlldataList.get(position).get("att_status"));
...
//                acvr.setText("Report");
                ...
                //map.setBackgroundResource(R.drawable.map_icons);
...
//                        System.out.println("Distance" + getAlldataList.get(position).get("distance") + "Amount" + getAlldataList.get(position).get("amount"));
...
//                        System.out.println("appointmentid:" + getAlldataList.get(position).get("appid"));
                ...
                appoint.setOnClickListener(new OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        Intent i=new Intent(getActivity().getApplicationContext(),appointmentoption.class);
...
//
    ...
    //public void notification(int year,int month,int day,int hour,int minutes,int seconds,int id){
...
//            System.out.println("from true if");
            getAlldataList = db.getAllTodayList1(db);
            // System.out.println("from true if");
            for (int i = 0; i < db.getAllTodayList1(db).size(); i++) {


                reportkey.add(db.getAllTodayList1(db).get(i).get("rkey"));
...
//                System.out.println("rkey" + getAlldataList.get(i).get("rkey"));
    ...
    private void viewDetails1() {
        ...
        reportkey0.clear();
        getAlldataList1 = db.getAllTodayList(db);
...
//            //Code to intitisliase db operation to get data of status info and check if owner,tenant,wi,w2 are all equal to yes.if yes ,print data.
            //else call notification.


            if(getAlldataList1.get(i).get("post_status").equals("null")) {

                System.out.println("tnotification");
                if (owner.equals("1")) {
                ...
                if (w1.equals("1")) {
                    System.out.println("Updated");
                } else if (w1.equals("0") || w1.equals("null")) {
                    w1_content = "Witness1";
                    //content = o_content+" "+t_content+" "+w1_content+" "+"biometric pending";
                }
                if (w2.equals("1")) {
                    System.out.println("Updated");
                } else if (w2.equals("0") || w2.equals("null")) {
                    w2_content = "Witness2";
                }
